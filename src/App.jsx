...
const css = `
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  .sev.skip     { background: rgba(39,174,96,.1);  color: var(--ok);       border: 1px solid rgba(39,174,96,.25); }
  ...
  /* ── Notification cards ── */
  .notif-stack { display: flex; flex-direction: column; gap: .75rem; margin-top: .5rem; }
  ...
  .slack-divider { border: none; border-top: 1px solid #e8d8f0; margin: .5rem 0; }
  ...
  .owner-emoji { font-size: 1.6rem; }
  .owner-name { font-weight: 600; font-size: .85rem; }
  .owner-role { font-size: .68rem; color: var(--muted); margin-top: .1rem; font-family: 'JetBrains Mono', monospace; }
  .owner-tags { display: flex; gap: .4rem; flex-wrap: wrap; margin-top: .5rem; }
  .owner-tag { font-size: .6rem; padding: .15rem .45rem; border-radius: 3px; background: var(--tag-bg); color: var(--muted); font-family: 'JetBrains Mono', monospace; }

  /* ── Escalation timer ── */
  .escalation-bar {
    margin-top: .75rem; padding: .75rem 1rem;
    border-radius: 8px; border: 1px dashed var(--high);
  ...
  .skip-card {
    border: 1.5px solid var(--border); border-radius: 10px;
    padding: 1.2rem; background: var(--surface2);
    display: flex; gap: .75rem; align-items: flex-start;
  }
  .skip-icon { font-size: 1.2rem; }
  ...
  .btn-reset {
    ...
    color: var(--muted); padding: .35rem .8rem; border-radius: 6px;
    cursor: pointer; font-size: .68rem; font-family: 'JetBrains Mono', monospace;
    transition: all .2s;
  }
...
function formatSlackMessage(analysis, owner) {
...
*Issue:* ${analysis.issue_key || "APP-XXX"} — ${analysis.summary}
...
export default function ReleaseAlertHub() {
  ...
  const trigger = async () => {
    ...
    const owner = comp ? TEAM_MATRIX[comp] : null;
...
Return ONLY valid JSON with these exact keys:
{
  ...
  "sprint_impact": "specific impact on current sprint/release in one sentence",
  "risk_summary": "2-3 bullet points (use • prefix) describing release risk",
  "recommended_action": "specific immediate action the team should take",
  "decision_deadline": "urgency framing e.g. 'Within 2 hours — release window closing'",
  "email_subject": "concise email subject line for owner notification",
  "email_body": "professional 3-4 sentence email body to the component owner and backup explaining the issue, its impact, and what action is needed from them. Address it to the owner by first name.",
  "escalation_window": "how long before auto-escalation e.g. '30 minutes'"
}`;
        ...
        <div className="layout">
          ...
          <div>
                    ...
                    return (
                      <div key={i}
                        ...
                        <div className="pill-dot" style={{
                          background: sev === "Critical" ? "var(--critical)" : "var(--high)"
                        }} />
                        {s.label}
                      </div>
                    );
            ...
            <div className="card" style={{ marginTop: "1.5rem" }}>
              <div className="card-header">
                <div className="card-title">👥 Team Ownership Matrix</div>
              </div>
                ...
                <div className="card-body">
                  <div className="skip-card">
                    ...
                    <div className="skip-text">
                      ...
                      Severity detected as <strong>{result.severity}</strong> — below the Critical/High threshold. This defect will follow the standard sprint triage process. No immediate notifications sent.
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "1.2rem" }}>
                  ...
                  <div className="card-body">
                    <div className="workflow">
                      {[
                        ...
                        { icon: "💬", label: "Slack alert posted to #release-decisions", desc: `${DECISION_MAKERS.length} decision makers notified`, cls: "ok" },
                ...
                <div className="card notif-card email">
                  ...
                  <div className="notif-body">
                    ...
                    <div style={{ whiteSpace: "pre-wrap", lineHeight: 1.7 }}>
                      {result.analysis.email_body}
                    </div>
                    <div style={{ marginTop: ".75rem", fontSize: ".68rem", color: "var(--muted)" }}>
                      — Release Alert System · Automated Notification
                    </div>
                  </div>
                </div>
                ...
                <div className="card notif-card slack">
                  <div className="notif-header">
                    <div className="notif-type">
                      <span style={{ fontSize: "1rem" }}>
                        <svg width="16" height="16" viewBox="0 0 54 54" fill="none" xmlns="http://www.w3.org/2000/svg" style={{display:"inline",verticalAlign:"middle"}}>
                  ...
                  <div className="notif-body">
                    <div className="slack-msg">
                      {formatSlackMessage(result.analysis, result.owner || { slack: "@unknown", team: "Unknown" })
                        .split("\n")
                        .map((line, i) => (
                ...
                <div className="escalation-bar">
                  ...
                  <div className="esc-text">
                    ...
                    If no acknowledgment received within {result.analysis.escalation_window}, this alert will automatically escalate to Engineering Director and VP of Product.
                  </div>
                </div>
              ...
              <div className="card">
                <div className="card-body" style={{ padding: "2.5rem", textAlign: "center" }}>
                  ...
                  <div style={{ fontSize: ".75rem", color: "var(--muted)", lineHeight: 1.6 }}>
                    ...
                    Critical and High severity defects trigger<br />the full notification workflow.
                  </div>
                </div>
              </div>
            )}
